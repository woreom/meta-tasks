...
from autoencoder_prototypical import PrototypicalAutoencoderNetwork
...
def proto_train(num_ways: int, num_shot: int, get_dataset: Callable, experiment_name: str,
          autoencoder_path: str="../outputs/exported/resnet_autoencoder/autoencoder_withouthist.pt"):
    print(f"\n ================== [PrototypicalNetwork][{experiment_name}] {num_ways}-WAY {num_shot}-SHOT ================== \n")
    ...
    model = PrototypicalNetwork(encoder= autoencoder.encoder)
    ...
    # best parameters lr=1e-4, epochs= 50, eval_step= 5, episode=1000, step_size=5,
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    #divide the learning rate by 2 at each epoch, as suggested in paper
    lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=10000, gamma=0.5, last_epoch=-1)
...
def proto_autoencoder_train(num_ways: int, num_shot: int, get_dataset: Callable, experiment_name: str,
          autoencoder_path: str="../outputs/exported/resnet_autoencoder/autoencoder_withouthist.pt"):
    ...
    model.fit(get_dataset=get_dataset, epochs= 5, train_num_episode=10000, test_num_episode= 10000,
                ...
                eval_step= 1, early_stop= True, optimizer= optimizer, lr_scheduler=lr_scheduler, loss= loss,
                output_path=save_dir,
                autoencoder_lr=1e-6, autoencoder_weight_decay= 1e-8,
                )
...
################################## MINI-IMAGENET ##################################
    ...
    # proto_train(num_ways= 5, num_shot= 5,
    #                         get_dataset= get_dataset, 
    #                         experiment_name= experiment_name,
    #                         autoencoder_path="../outputs/exported/resnet_autoencoder/autoencoder_withouthist.pt"
    #                         )
    
    proto_autoencoder_train(num_ways= 5, num_shot= 5,
                            get_dataset= get_dataset, 
                            experiment_name= experiment_name,
                            autoencoder_path="../outputs/exported/resnet_autoencoder/autoencoder_withouthist.pt"
                            )

    # proto_train(num_ways= 5, num_shot= 1,
    #                         get_dataset= get_dataset, 
    #                         experiment_name= experiment_name,
    #                         autoencoder_path="../outputs/exported/resnet_autoencoder/autoencoder_withouthist.pt")
    
    proto_autoencoder_train(num_ways= 5, num_shot= 1,
                            get_dataset= get_dataset, 
                            experiment_name= experiment_name,
                            autoencoder_path="../outputs/exported/resnet_autoencoder/autoencoder_withouthist.pt")
